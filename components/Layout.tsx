...
const Layout: React.FC<LayoutProps> = ({ onVoiceToggle, onSosTrigger }) => {
  ...
  const navItems = [
    ...
    { label: 'Adult', icon: 'security', path: '/vault/adult' },
    { label: 'Operator', icon: 'admin_panel_settings', path: '/vault/operator' },
    { label: 'SOS', icon: 'emergency', path: '/lockdown', isSos: true },
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24">
      ...
      <button
        ...
        className="fixed bottom-24 right-6 w-16 h-16 bg-primary text-white rounded-full shadow-2xl shadow-primary/40 flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-50 group border-2 border-white/10"
      >
